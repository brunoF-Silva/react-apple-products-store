...
const MacBook = () => {
  ...
  p-8 h-full"
    >
      <h1
        className="2xl:text-4xl xl:text-3xl md:text-2xl font-semibold
    text-gray-800 mb-8"
      >
      ...
      <div className="flex">
        {macbookModels.map((mb, index) => (
          <button
            key={index}
            className={`2xl:w-36 md:w-26 w-18 2xl:h-18 md:12 h-12 flex
        flex-col justify-evenly items-center border 
        ${index === 0 ? "rounded-l-xl" : "rounded-r-xl"}
        ${index === selectedChip ? "boder-2 border-blue-400" : "border-gray-800"}
        `}
            onClick={() => handleSelectedChip(index)}
          >
            <span
              className="2xl:text-lg xl:text-sm text-xs font-bold
          text-gray-800 "
            >
              With {mb.chip} chip
            </span>
      ...
      <div className="flex flex-col 2xl:p-4 md:items-start items-center">
        ...
        <span
          className="2xl:text-lg xl:text-sm font-medium text-gray-800
        mb-2 text-center md:text-left "
        >
          {currentColor.label}
        </span>
        <div
          className="flex space-x-2 mb-4 md:text-left justify-center
        md:justify-start"
        >
          {currentMb.colors.map((col, index) => (
            <button
              ...
              className={`2xl:w-6 sm:w-5 2xl:h-6 sm:h-5 h-4 bg-[#2e3641]
            rounded-full border hover:border-blue-500 
            ${
              selectedColor === index
        ...
        <button
          type="button"
          className="w-min text-sm py-1 px-3 bg-blue-400
