...
const IMac = () => {
  ...
  return (
    <div
      className="h-full grid grid-cols-2 grid-rows-2 md:gap-4 gap-1
    bg-white relative"
    >
      <div
      ...
      >
        <img
          ...
          alt="IMac"
          className="2xl:max-w-full xl:max-w-5 lg:max-w-48"
        />
      </div>
      <div
        className="bg-gray-50 col-span-1 row-span-1 flex items-center
      ...
      <div
        className="absolute md:top-1/2 top-3/4 md:left-4 left-12 transform -translate-y-1/2
      flex flex-col lg:space-y-2 space-y-1 bg-gray-50"
      >
          ...
          <div key={index} className="flex items-center space-x-2">
            <button
              ...
              className={`2xl:w-8 md:w-5 w-4 2xl:h-8 md:h-5 h-4 rounded-full
                flex items-center justify-center ${selectedColor === model.label.toLowerCase() && 'border-2 border-blue-500'}`}
                onClick={() => {
        ...
        <button
          className="text-sm mt-4 w-min py-1 px-2 bg-blue-400
